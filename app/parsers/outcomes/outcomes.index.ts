...
export let OutcomeParsingMapPromise: Promise<{[key: string]: Parser}> =
  new Promise((a, r) => {
    ...
    fs.readdir(pathToRoutes, (err: any, files: string[]) => {
      ...
      let fHandler = (file: string) => {
        ...
        if (fName.length) {
          ...
          try {
            ...
            if (!obj.getParser) {
              logger.warn(`Not loading outcome parser ${file} since it doesnt have getParser method or object`);
      ...
      files
        .filter((file: string) => path.extname(file) === '.js' && path.basename(file, '.js').endsWith('parser'))
        .forEach(fHandler);
      a(res);
    });
  });
