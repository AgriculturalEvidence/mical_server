...
abstract class Parser {
  ...
  abstract get extraCols(): ColumDesc;
  ...
  findColumns(wb: WorkBook, colMap: {[key: string]: string}): [boolean, WorkSheet, ColumnMapping] {
...
abstract class PaperParser extends Parser {
  ...
  abstract get importID(): string;

  get extraCols() {
    return {...this.filterCols, ... this.infoCols}
  };

  async prepareRow(ws: WorkSheet, 
                              colInfo: ColumDesc, 
  ...
  async getIntervention(ws: WorkSheet, colInfo: ColumDesc, rowIdx: number) {
